...
export default async function ResultPage({
  ...
  // sessionIdが指定されてぁE��ばそ�EセチE��ョンを、なければ最新を取征E  let query = supabase
    .from('sessions')
    .select('*, tests(*)')
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-blue-50 p-4">
        <div className="bg-white rounded-2xl shadow p-8 text-center max-w-sm w-full space-y-3">
          ...
          <a href="/student" className="text-blue-600 text-sm hover:underline">ホ�Eムに戻めE/a>
        </div>
      </div>
    )
  }
      ...
      <div className="bg-white rounded-2xl shadow-lg w-full max-w-sm overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="bg-gray-50 rounded-xl p-4 space-y-1">
            <p className="text-xs text-gray-400">受験老E/p>
          ...
          <div className="space-y-3">
            ...
            {test.pass_score !== null && (
              <div className="flex justify-between items-center py-2 border-b border-gray-100">
                ...
                <span className="text-gray-700">{test.pass_score}点以丁E/span>
              </div>
            )}

            {pointsEarned !== null && (
              <div className="flex justify-between items-center py-2 border-b border-gray-100">
                <span className="text-gray-600 text-sm">獲得�EインチE/span>
                <span className="font-bold text-blue-600 text-lg">{pointsEarned}pt</span>
              </div>
            )}

            <div className="flex justify-between items-center py-2 border-b border-gray-100">
              <span className="text-gray-600 text-sm">提�E時刻</span>
          ...
          {pointsEarned !== null && (
            <div className={`rounded-xl p-4 text-center ${
              ...
              {pointsEarned === 12 ? (
                <p className="text-yellow-700 font-bold">パ�Eフェクト！E+12pt</p>
              ) : pointsEarned >= 8 ? (
                <p className="text-yellow-700 font-medium">素晴らしぁE��E+{pointsEarned}pt</p>
              ) : pointsEarned >= 4 ? (
                <p className="text-blue-700 font-medium">よくできました +{pointsEarned}pt</p>
              ) : pointsEarned > 0 ? (
                <p className="text-green-700 font-medium">ポイント獲征E+{pointsEarned}pt</p>
              ) : (
                <p className="text-gray-500 text-sm">84点以上でポイントが獲得できまぁE/p>
              )}
            </div>
          )}
